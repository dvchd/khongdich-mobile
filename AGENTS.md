...
- Image emulator: `google_apis` (API 35) GMS bị hạn chế — **Google Sign-In luôn fail `[16] Account reauth failed`** khi account cần reauth (login "chập chờn" là do token account hết hạn → reauth không chạy được). Muốn test đăng nhập Google phải dùng image `google_apis_playstore` (GMS đầy đủ, như thiết bị thật): `Pixel_6_Pro_API_33_Play`. Xem `~/.android/avd/*.avd/config.ini` dòng `image.sysdir.1` để biết image nào.
