...
## Trạng thái hiện tại (v0.13.0)

**Build:** `flutter analyze` → 0 lỗi · 268 tests xanh · CI chạy analyze + test song song trên mọi push/PR; push tag `v*` → build APK + AAB prod (cache Gradle) → GitHub Releases + tự upload AAB lên Play Console (track Closed Testing).
